...
## Summary
We developed the FLAMINGO to reconstruct the high-resolution 3D genome structures based on the chromatin contact maps with high scalability. By integrating 1D epigenomics dataset, FLAMINGO is able to make cross-cell-type predictions and predict high-resolution 3D structures from low-resolution contact maps.
...
The pipeline of the algorithm consists of 9 pieces, including data preprocessing and modelling. A wrapper is provided to run the whole pipeline for each task and the user only needs to provide the path to the data. A detailed description of each piece is provided.
