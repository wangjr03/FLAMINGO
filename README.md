...
The 3D structure of all 23 chromosomes could be found [here](https://docs.google.com/presentation/d/1dJXx5xVuXCd9MYfpq8AJwn1UPTieAgFbcdfJlnd8AbI/edit?usp=sharing). <br>
...
2. Cell-type specific DNase-seq data: The cell-type DNase-seq data should be stored in the bedGraph format with at least four columns. This input is only required by iFLAMINGO and could be ignored for FLAMINGO.
...
5. *5_impute_DNase_dist.R*: This script is used to predict pairwise distance with DNase signals of two DNA fragments  and the 1D genomic distances based on a pre-trained regression model. This part is only used by iFLAMINGO and could be skipped if no epigenomics data required.<br>
