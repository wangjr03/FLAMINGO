...
## Dependencies
The implementation of the algorithm is based on bedtools and  R 3.5.1. It depends on four R packages: strawr, parallel, mgcv and Matrix.
...
The stand-alone scripts for all steps of FLAMINGO are provided below. The stand-alone version is especially usefull when users have access to HPC, which can run hundreds of small jobs in parallel.
### Input data
...
## Relevant dadta to reproduce the analysis
