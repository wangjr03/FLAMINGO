...
#8 parameters:
#1. fraction of basis
#2. penalization of DNase term
#3. distance penalization lower bound
#4. input pariwise distance
#5. input interaction frequency
#6. fragment id
#7. output path
#8. DNase input data
...
#args <- c('0.25','1','0.3','/mnt/ufs18/rs-027/compbio/wanglab/haowang/Proj10_recon_3D/data/chr21_1mb_frag/Dist_frag28.txt',"/mnt/ufs18/rs-027/compbio/wanglab/haowang/Proj10_recon_3D/data/chr21_1mb_frag/IF_frag28.txt",'NA','NA',"/mnt/ufs18/rs-027/compbio/wanglab/haowang/Proj10_recon_3D/data/chr21_1mb_frag/DNase_3D_impute28.txt")
...
dnase <- read.table(args[8])
dnase <- as.matrix(dnase)
dnase <- dnase^2
...
grad <- function(P,gamma){
  ...
  t4 <- Sys.time()
  d_term = 2*lambda_2*new_calculate_adj(a-dnase_vec)%*%P
...
d_list <- parLapply(cl,1:n_omega,function(x){cbind(convert_index(omega[x,]),x)})
...
dnase_vec <- apply(omega,1,function(x){
  dnase[x[1],x[2]]
})
...
while(iter < 2){
  ...
  P_p = p_t
  iter <- iter + 1
  save.image(paste0(output_pth,"/5kb_frag",args[6],'.Rdata'))
