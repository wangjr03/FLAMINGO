#' iflamingo.reconstruct_structure_worker
#'
#' Reconstruct the 3D genome structure using iFLAMINGO algorithm
#' @param input_if Input interaction frequency matrix.
#' @param pd Input pairwise distance matrix.
#' @param dist_epi Input inputed distance based on the epigenomic features 
...
#' @export
iflamingo.reconstruct_structure_worker <- function(input_if,pd,dist_epi,sw,lambda,lambda_epi,max_dist,nThread=2){
  library(parallel)
  library(mgcv)
  library(Matrix)
  # process input data
  ...
  b = cal_proj(omega,M)
  epi_vec <- apply(omega,1,function(x){
    dist_epi[x[1],x[2]]
  })
  ...
  re_error <- 10
  r = 1
  p_t <- iflamingo_sub_prob(gamma_p,cl,b,d,epi_vec,omega,omega_diag,func_list,func_list_diag,all_elements,all_elements_diag,n,q,lambda,lambda_epi,r)
