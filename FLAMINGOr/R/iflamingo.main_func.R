...
#' @param epigenomic_file Path to the epigenimic data in bedgraph format.
...
#' @param lambda_epi Lagrigian coefficient for the epigenomic term.
...
iflamingo.main_func <- function(hic_data_low,file_format,epigenomic_file,domain_res,frag_res,chr_size,chr_name,normalization,downsampling_rates,lambda,lambda_epi,max_dist,nThread,alpha=-0.25,max_iter=500,hic_data_high=NULL,norm_low=NULL,norm_high=NULL){
  ...
  print('Processing epigenimic data')
  flamingo.calculating_epigenomic_signal(epigenomic_file=epigenomic_file)
  flamingo.impute_dist_from_epi(frag_res=frag_res)
  ...
  flamigo_intra_domain_prediction = iflamingo.reconstruct_structure(sw=downsampling_rates,lambda = lambda,lambda_epi=lambda_epi,max_dist = max_dist,nThread=nThread)
